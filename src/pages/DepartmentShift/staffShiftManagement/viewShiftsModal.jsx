...
const ViewShiftsModal = ({ open, onClose, staff, assignments, onUpdate }) => {
    ...
    const handleDelete = async (assignmentId) => {
        try {
            ...
            toast.error(
                error.response?.data?.EM || "Failed to delete shift",
                { autoClose: 2000 }
            );
        } finally {
            setUpdating(null);
    ...
    const handleUpdateStatus = async (assignmentId, newStatus, statusLabel) => {
        try {
            ...
            if (res.EC === 0) {
                toast.success(
                    res.EM || `Shift ${statusLabel} successfully`,
                    { autoClose: 2000 }
                );
                onUpdate();
            } else {
                toast.error(
                    res.EM || `Failed to ${statusLabel.toLowerCase()} shift`,
                    { autoClose: 2000 }
                );
            }
        } catch (error) {
            ...
            <Modal
                title={
                    <div className="modal-title">
                        ...
                        <span>
                            Shift Assignments - {staff?.personalInfo?.fullName}
                        </span>
                    </div>
                }
                open={open}
