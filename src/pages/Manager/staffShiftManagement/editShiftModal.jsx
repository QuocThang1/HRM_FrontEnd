...
const EditShiftModal = ({ open, onClose, assignment, staff, onSuccess }) => {
  ...
  useEffect(() => {
    if (open && assignment) {
      ...
      form.setFieldsValue({
        ...
        dateRange: [dayjs(assignment.fromDate), dayjs(assignment.toDate)],
      });
    }
  }, [open, assignment]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await updateShiftAssignmentApi(assignment._id, updateData);
      ...
      toast.error(error.response?.data?.EM || "Failed to update shift", {
        autoClose: 2000,
      });
    } finally {
      setSubmitting(false);
